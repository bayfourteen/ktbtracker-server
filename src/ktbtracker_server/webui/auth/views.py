...
from ktbtracker_server.config import firebase
from ktbtracker_server.config.firebase import signin_with_email_password
...
async def login_post(
    ...
    try:
        user = await signin_with_email_password(request, email, password)
        ...
        # Handle when invalid credentials were provided...
        if e.response.status_code == 400 and e.response.json().get("error", {}).get("message") == "INVALID_LOGIN_CREDENTIALS":
            return templates.TemplateResponse("login/index.html", context={
                "request": request,
                "next_url": next_url,
                "error": _("INVALID_LOGIN_CREDENTIALS"),
                "page_background": "bg-primary"
            })

        logger.error(e, exc_info=True)
        return RedirectResponse(url=f'/login?error={quote(str(e))}{'&next_url=' + next_url if next_url else ''}', status_code=status.HTTP_303_SEE_OTHER)
